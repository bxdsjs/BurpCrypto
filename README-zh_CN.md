...
# 安装
BurpCrypto可从本仓库的[Releases](https://github.com/bxdsjs/BurpCrypto/releases/tag/v0.1.9.1-montoya)页面下载已编译好的版本，或下载源代码本地编译，然后在BurpSuite的扩展列表中添加插件，等待Output中输出`BurpCrypto loaded successfully!`则表示插件加载成功。
...
K/V数据库清理较为简单，首先需将BurpSuite完全退出，然后手动删除临时目录下的`BurpCrypto_*.ldb`文件夹即可
